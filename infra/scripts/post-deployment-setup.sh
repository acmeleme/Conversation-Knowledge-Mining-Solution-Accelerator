...
MANAGED_IDENTITY_NAME=$(az identity list --resource-group "$RESOURCE_GROUP" --query "[?contains(name, 'id-')].name" -o tsv | head -1)
...
pip install -q -r infra/scripts/index_scripts/requirements.txt
...
python infra/scripts/index_scripts/01_create_search_index_manual.py "$KEY_VAULT_NAME" "$MANAGED_IDENTITY_CLIENT_ID"
